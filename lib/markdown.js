...
this.Markdown = function(obj) {
	...
	if (strip) md = stripUnwantedHTML(obj);
	md =  Showdown.makeHtml(md);
	return md;
}
 ...
 **/
var stripUnwantedHTML = function(obj) { //html , allowedTags, allowedAttributes, forceProtocol, escapeTags
    var allowedTags = obj.allowedTags || 
            'a|b|blockquote|code|del|dd|dl|dt|em|h1|h2|h3|'+
            'i|img|li|ol|p|pre|sup|sub|strong|strike|ul|br|hr',
        allowedAttributes = obj.allowedAttributes || {
            'img': 'src|width|height|alt',
            'a':   'href',
            '*':   'title'
        }, forceProtocol = obj.forceProtocol || false, escapeTags = obj.escapeTags || false, html = obj.text;
    ...
    return html.replace(findTags, function(original, lslash, tag, params){
        ...
        if(!tag.match(testAllowed))
           return (escapeTags) ? original.replace(/</g, "&lt;").replace(/>/g, "&gt;") : "";
